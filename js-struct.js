 ...
 */
	int8: {
		value: function value(name, endian) {
 ...
 */
	string: {
		value: function value(name, length) {
			var code = "(function(o) {\n";
			code += "   var str = \"\";\n";
 ...
 */
	array: {
		value: function value(name, type, length) {
			var code = "(function(o) {\n";
 ...
 */
	struct: {
		value: function value(name, struct) {
			return {
				name: name,
 ...
 */
	create: {
		value: function value() /* collected via arguments */ {
			var type;
					...
					for (var i = 0; i < count; ++i) {
						...
						s = (function (o) {
							...
							for (var i = 0; i < functionArguments.length; ++i) {
								type = functionArguments[i];
								if (!type.structProperty) {
									continue;
								}
								if (type.name) {
									Object.defineProperty(struct, type.name, { value: type.defaultValue, enumerable: true, configurable: true, writable: true });
									st[type.name] = eval(type.readCode);
								}
